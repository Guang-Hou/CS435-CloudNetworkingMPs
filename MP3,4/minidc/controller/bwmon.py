...
import sys
sys.path.append("./minidc/controller/")
...
class BandwidthMonitor(app_manager.RyuApp):
    ...
    def statsReplyHandler(self, ev):
        ...
        # [ ADD YOUR CODE HERE ]
        # print("name: ", name)
        # print("self.topo.ports: ", self.topo.ports)

        if name in self.topo.edgeSwitches:
            for stat in body:
                port = stat.port_no
                # print("port: ", port)
                if port in self.topo.ports[name].keys():
                    received = stat.rx_bytes
                    transmitted = stat.tx_bytes
                    hostname = self.topo.ports[name][port]
                    # print("hostname: ", hostname)
                    if hostname in self.topo.hosts:
                        self.bwstats.addHostBwStat(hostname, transmitted, received)
